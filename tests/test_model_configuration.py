...
class TestModelConfiguration:
    ...
    def test_model_configuration_raises_if_state_channels_are_not_positive(
        ...
        with raises(ValueError):
            ModelConfiguration(state_channels, 1, True, 0, 0.0, 1, 0.1, 0.5,
                               True)
    ...
    def test_model_configuration_intermediate_channels_are_not_positive(
        ...
        with raises(ValueError):
            ModelConfiguration(1, intermediate_channels, True, 0, 0.0, 1, 0.1,
                               0.5, True)

    @mark.parametrize('padding', ["notvalid", "samewise" ])
    def test_model_configuration_raises_if_padding_is_invalid(
        self, padding: str
    ):
        with raises(ValueError):
            ModelConfiguration(1, 1, True, padding, 0.0, 1, 0.1, 0.5, True)
    ...
    def test_model_configuration_raises_if_rotation_is_out_of_bounds(
        ...
        with raises(ValueError):
            ModelConfiguration(1, 1, True, 0, rotation, 1, 0.1, 0.5, True)
    ...
    def test_model_configuration_raises_if_step_size_is_not_positive(
        ...
        with raises(ValueError):
            ModelConfiguration(1, 1, True, 0, 0.0, step_size, 0.1, 0.5, True)
    ...
    def test_model_configuration_raises_if_threshold_is_out_of_bounds(
        ...
        with raises(ValueError):
            ModelConfiguration(1, 1, True, 0, 0.0, 0.5, threshold, 0.5, True)
    ...
    def test_model_configuration_raises_if_update_rate_is_out_of_bounds(
        ...
        with raises(ValueError):
            ModelConfiguration(1, 1, True, 0, 0.0, 0.5, 0.1, update_rate, True)
